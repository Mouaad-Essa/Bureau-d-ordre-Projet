...
export async function POST(request: Request) {
  try {
    ...
    const idDepart = request.headers.get("idDepart");
const files = formData.getAll("files") as File[];

    if (!idDepart) {
      console.error("❌ Missing Depart ID");
      return new Response(JSON.stringify({ error: "Missing Depart ID" }), { status: 400 });
    }
    ...
    for (const file of files) {
      ...
      const filePath = join(process.cwd(), "public/uploads/departs", fileName);
